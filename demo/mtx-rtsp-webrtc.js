class StreamController {
  ...
  initReqTools() {
    ...
    const reqTools = (this.reqTools = {
      ...
      publishSource: async (url, toh264 = false) => {
        const streamId = this.getStreamId(url);
        const source_rul = (url + "").replace("#", "%23"); // 处理 # 符号
        // 当需要转码时，使用 ffmpeg 转码
      ...
      getRTCIframeUrl: async (url, toh264 = false) => {
        const streamId = this.getStreamId(url);
        const res1 = await reqTools.getPath(url);
        if (!res1.ok) {
  ...
  static getInstance() {
    const instance = StreamController.instance = StreamController.instance || new StreamController();
    return instance;
  }

  static mountPlayer(config = {}) {
    StreamController.getInstance().mountPlayer(config);
  }
  static stopStream(url) {
    StreamController.getInstance().stopStream(url);
  }
}
